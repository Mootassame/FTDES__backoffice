...
const publicationListActions = {
  ...
  doExport: () => async (dispatch, getState) => {
    try {
      ...
      dispatch({
        type: publicationListActions.EXPORT_STARTED,
      });
      ...
      new Exporter(
        exporterFields,
        i18n('entities.publication.exporterFileName'),
      ).transformAndExportAsExcelFile(response.rows);

      dispatch({
        type: publicationListActions.EXPORT_SUCCESS,
      });
    } catch (error) {
      Errors.handle(error);

      dispatch({
        type: publicationListActions.EXPORT_ERROR,
      });
    }
  },
